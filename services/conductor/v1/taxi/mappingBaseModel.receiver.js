...
const TASK_NAME = 'TMS_MAPPING_BASE_MODEL_FOR_TAXI'
  ...
  await conductorClient.registerWatcher(
    ...
    async (data, updater) => {
      ...
      try {
        ...
        const paymentFare = R.path(['inputData', 'orders', 'payment', 'detailService', 'fare'], data)
        
        if (!paymentFare || paymentFare === '0') {
          return updater.fail({
            taskId: data.taskId,
            outputData: {
              ...data.inputData,
              statuses: [
                ...
                { status: 'ORDER_CANNOT_MAPPING_PAYMENT_FARE', updatedAt: Date.now() },
              ],
