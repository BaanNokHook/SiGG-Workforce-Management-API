...
const TASK_NAME = 'TMS_CREATE_TASKS'
...
export default async () => {
  await conductorClient.registerWatcher(
    ...
    async (data, updater) => {
      ...
      try {
        let tasks = R.path(['inputData', 'orders', 'tasks'], data)
        ...
        try {
          tasks = tasks.map(task => {
            const _passengers = task.passengers.map(passenger => ({
              ...passenger,
          ...
          const passengers = R.flatten(R.pluck('passengers')(tasks))
          await passengerRepository.bulkUpsert(['_id'], passengers)
        } catch (error) {
          _graylog.error('ERROR.PASSENGERS_BULK_WRITE_FAILED', { error })
          throw Throw.BULK_WRITE_FAILED({ message: 'Failure bulk upsert passengers', error })
        }
